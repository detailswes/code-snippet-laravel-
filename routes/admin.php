...
Route::controller(LoginController::class)->middleware(['guest'])->group(function () {
    ...
    Route::post('login', 'login')->middleware('throttle:5,1')->name('admin.post.login');
});

Route::controller(ForgotPasswordController::class)->middleware(['guest'])->group(function () {
    Route::get('forget-password', 'forgetPassword')->name('forget.password');
    Route::post('forget/password/request', 'forgetPasswordRequest')->middleware('throttle:5,1')->name('forget.password.request');
    Route::get('reset-password/{token}', 'ResetLoginPassword')->name('ResetPassword');
    Route::post('update-password', 'changePassword')->middleware('throttle:5,1')->name('change.password');
});
